...
**Limitação reconhecida e justificativa da seleção:** a literatura de C2 não produz, até o momento, trabalhos que proponham diretamente uma arquitetura de Data Lake ou Data Lakehouse para ambientes operacionais militares. Nenhum dos cinco artigos selecionados cumpre esse papel isoladamente. A fundamentação arquitetural do instrumento tecnológico proposto — ingestão em camadas (*Bronze/Silver/Gold* ou equivalente), catálogo de metadados, esquema sob demanda (*schema-on-read*), padrões de governança e qualidade de dados — é construída a partir da literatura própria de engenharia de dados, coberta em seção distinta desta pesquisa.

O que os cinco trabalhos de C2 selecionados oferecem é o **lado da demanda**: evidenciam *por que* os sistemas de C2 precisam de uma infraestrutura de dados unificada (Verhoosel; Peerdeman), *o que* a IA precisará fazer com esses dados no ciclo de planejamento (Schubert; Abdelazez; Turner) e *como* soluções dessa natureza devem ser avaliadas em contexto operacional real (Turner). Em conjunto, eles fundamentam o *problema* e os *requisitos funcionais* que a proposta de Data Lake deve atender, estabelecendo o elo entre a arquitetura tecnológica proposta e o domínio operacional que ela serve.
...
| **Alinhamento arquitetural** | Discute infraestrutura de dados, integração de fontes heterogêneas, arquitetura de C2 orientada a dados, ou requisitos funcionais que uma arquitetura de dados deve atender | Foco exclusivo em hardware, comunicações, plataformas autônomas ou cibernética |
...
| --- | --- | --- | --- | --- |
| **1** | Schubert, Brynielsson, Nilsson, Svenmarck | 2018 | Artificial Intelligence for Decision Support in Command and Control Systems | Único trabalho do corpus com dados empíricos coletados junto a militares sobre uso de IA nos processos de inteligência e operações. Discute diretamente o apoio a COP, análise de ameaças e avaliação de linhas de ação — funções equivalentes às dos seções S2 e S3 no PPCOT. Fundamenta *o que* a IA precisa fazer com os dados do Data Lake. |
| **2** | Verhoosel, Nouwt, Voogd | 2020 | Data-driven, Service-oriented, Knowledge-based C2 | Propõe arquitetura de serviços para integração de bases de conhecimento heterogêneas e distribuídas — problema central desta dissertação. Cita explicitamente o conceito de "data-lake layer" como alternativa comparativa, conferindo ancoragem direta ao tema central da pesquisa. Fundamenta *a lógica de integração* que o Data Lake deve implementar. |
| **3** | Peerdeman et al. | 2024 | Architectural Concepts for Future C2 Systems in MDO (NATO SAS-IST 171) | Único trabalho do corpus com discussão direta de componentes de arquitetura de dados para C2: cita o "NCDF Data Lake" da NATO e a distinção *scheme-on-write/scheme-on-read* — princípio fundamental de Data Lakes. Apresenta princípios FAIR e VAULTIS do DoD para governança de dados militares e propõe Smart Connector e Service Directory com metadados de qualidade e classificação — equivalentes à camada de catálogo e governança do Data Lake proposto. |
| **4** | Abdelazez, Garber, Ghanmi | 2023 | Decision Support for Commanders: AI Approach to Wargaming | Trata diretamente da aplicação de IA para exploração de Linhas de Ação e condução do Jogo da Guerra — equivalentes ao Step 4 do PPM. Menciona explicitamente que o Ministério da Defesa do Brasil planeja incorporar IA ao processo de wargaming de COAs, conectando-se ao contexto nacional desta pesquisa. Fundamenta *o caso de uso primário* que o Data Lake deve habilitar. |
...
| 10 | Simpson, El Sawah, Oosthuizen, Abbass | 2021 | Agile, Antifragile, AI-Enabled C2 (A3IC2) | Framework teórico sobre riscos de fragilidade na integração de IA em C2. O argumento central — IA como amplificadora do Comandante — já é coberto empiricamente por Schubert et al. (incluído). Não propõe nenhum componente de arquitetura de dados; seu eixo principal (antifragilidade, caos computacional, função allocation) não contribui para o problema de ingestão, governança ou estrutura do Data Lake proposto. |
...
| 20 | Simpson, M. L. Jr. | 2019 | MDC2 an Execution Concept | Propõe uma visão de execução para o MDC2. O paper critica explicitamente a ideia de que um "data lake" compartilhado, por si só, melhora a qualidade da decisão — argumento que reforça a necessidade de maturidade no uso dos dados, não de sua rejeição. Porém, a contribuição conceitual não supera as dos artigos já selecionados. |
...
### Artigo 1 — O Que a IA Deve Fazer com os Dados: Evidência Empírica dos Processos S2 e S3
...
Para esta dissertação, Schubert et al. respondem a uma pergunta crucial: *o que, concretamente, a IA precisará fazer com os dados armazenados no Data Lake?* Os autores identificam as funções cognitivas de C2 que a IA pode apoiar — análise de situação, avaliação de COAs, síntese do COP — fornecendo os **requisitos funcionais de uso** que a arquitetura de dados deve ser capaz de servir. A conclusão central — de que a IA confere vantagem operacional à força que conseguir analisar informações mais rapidamente — alinha-se à hipótese central desta dissertação.

---

### Artigo 2 — A Lógica de Integração: Serviços de Dados Heterogêneos e a Menção ao Data Lake
...
Essa menção direta ao conceito de Data Lake ancora conceitualmente o instrumento tecnológico proposto nesta dissertação dentro do debate arquitetural do campo de C2. Para além da nomenclatura, a arquitetura inclui um componente de diretório semântico de serviços (*Semantic Service Directory*) e um conector inteligente que habilita a descoberta e integração automáticas de novas fontes de dados — lógica análoga à camada de ingestão e catálogo do Data Lake proposto. Verhoosel et al. fundamentam, portanto, *a lógica de integração de dados* que o Data Lake deve implementar no contexto de C2.

---

### Artigo 3 — A Base Arquitetural: Data Lake Militar, Metadados, FAIR e Governança de Dados

> **PEERDEMAN, M. et al.** Architectural Concepts to Support Future C2 Systems that Enable Multi-Domain Operations. *29th International Command and Control Research & Technology Symposium*, 2024.

**Justificativa:** Este é o trabalho do corpus que mais se aproxima de uma proposta concreta de arquitetura de dados para sistemas de C2 militares, sendo o único a mencionar explicitamente um **Data Lake em operação no contexto de defesa**. Os autores, membros do grupo NATO SAS-IST 171 (TNO, Fraunhofer, DRDC, Leonardo, NCIA), descrevem os componentes arquiteturais necessários para suportar Operações Multidomínio, partindo das metas de dados da estratégia do DoD norte-americano.

A leitura completa revelou contribuições diretamente aplicáveis a esta dissertação em três dimensões:

**1. Fundamento conceitual do Data Lake:** O paper distingue explicitamente as arquiteturas *scheme-on-write* (bancos de dados transacionais tradicionais) das arquiteturas *scheme-on-read* — princípio fundamental de Data Lakes, onde o esquema é aplicado no momento da consulta, não da ingestão. Os autores citam nominalmente o **"NCDF Data Lake"** (*NATO Core Data Framework Data Lake*), desenvolvido pelo NATO Data Management Capability Team (DMCaT), como a implementação de referência para armazenamento centralizado de dados de diferentes sistemas em formato padronizado com semântica harmonizada.

**2. Gestão de metadados e governança:** O paper identifica que arquiteturas de dados distribuídas exigem "far-reaching exchange or provision of metadata, including classification labeling, quality metadata" — precisamente os componentes de governança que a camada Silver do Data Lake proposto deve implementar. Os **princípios VAULTIS** do DoD (Visible, Accessible, Understandable, Linked, Trustworthy, Interoperable, Secure) e os **princípios FAIR** (Findable, Accessible, Interoperable, Reusable) são apresentados como o framework de governança de dados aplicável ao contexto militar — referencial diretamente utilizável na dissertação para justificar os critérios de qualidade e acessibilidade dos dados do PITCIC.

**3. Componentes arquiteturais transferíveis:** O *Smart Connector* (baseado em SPARQL e LLM para descoberta automática de fontes) e o *Service Directory* com metadados sobre serviços de dados correspondem, funcionalmente, à camada de catálogo e à interface de consulta de um Lakehouse. A ontologia MIP (*Military Information Model*) como modelo semântico de domínio C2 é análoga à camada de metadados semânticos que organiza os dados do PITCIC.

Para esta dissertação, Peerdeman et al. fornecem a **ponte conceitual entre a literatura de Data Lake** (coberta separadamente) **e o domínio de C2**: provam que o problema de arquitetura de dados em sistemas militares é reconhecido, debatido e parcialmente endereçado na academia de C2, o que legitima a proposta desta pesquisa como contribuição ao estado da arte.

---

### Artigo 4 — O Caso de Uso Central: IA para Linhas de Ação e o Jogo da Guerra
...
Para esta dissertação, Abdelazez et al. fundamentam **o caso de uso primário que o Data Lake deve habilitar**: a geração e avaliação automatizada de COAs a partir dos dados do PITCIC. Os autores demonstram como a IA pode gerar COAs inéditas (analogia com AlphaGo), executar múltiplas iterações inviáveis no wargaming manual e funcionar como árbitro via aprendizado por reforço. Isso implica que os dados de terreno, meteorologia, força inimiga e civis (PITCIC) precisam estar disponíveis, estruturados e acessíveis em um formato que algoritmos de IA consigam processar — requisito de *prontidão de dados* (*data readiness*) que justifica diretamente a proposta de Data Lake desta pesquisa.

---

### Artigo 5 — Como Avaliar: IA para Design Operacional e Metodologia Sociotécnica
...
**Justificativa:** Este artigo documenta a avaliação formativa de soluções de IA para planejamento operacional no âmbito do projeto *Machine Speed Command and Control* (MSC2) do DSTL britânico. A leitura completa confirmou que a pesquisa avalia dois conceitos específicos: (a) mapeamento de perspectivas de múltiplos atores (*Stakeholder Mapper*) e (b) suporte ao *Design Operacional* — geração de *Decisive Conditions* como parte da estrutura de planejamento. Esta segunda função corresponde diretamente ao *Design Operacional* do PPM brasileiro.

Para esta dissertação, Turner et al. contribuem em duas dimensões complementares. **Primeiro**, documentam que os planejadores militares identificaram como principal barreira à adoção de IA no planejamento operacional a **indisponibilidade e heterogeneidade dos dados** — o que valida empiricamente o problema de pesquisa desta dissertação. **Segundo**, a metodologia sociotécnica empregada — que combina ex-planejadores militares, pesquisadores de C2 e desenvolvedores de IA em avaliações formativas antes do comprometimento de recursos de desenvolvimento — oferece modelo aplicável à etapa de Avaliação do ciclo DSR desta dissertação. Turner et al. tornam-se, assim, referência simultânea para o referencial teórico de C2 e para a estratégia metodológica.

---

## 4. Síntese: Cobertura Coletiva dos Cinco Artigos

| # | Autores | Ano | Título (resumido) | Eixo de contribuição para a dissertação |
| --- | --- | --- | --- | --- |
| 1 | Schubert et al. | 2018 | AI for Decision Support in C2 Systems | *Requisitos funcionais*: o que a IA deve fazer com os dados (S2/S3, COP, análise de ameaças) |
| 2 | Verhoosel et al. | 2020 | Data-driven, Knowledge-based C2 | *Lógica de integração*: como dados heterogêneos devem ser conectados em C2; âncora conceitual ao Data Lake |
| 3 | Peerdeman et al. | 2024 | Architectural Concepts for MDO C2 | *Base arquitetural*: Data Lake militar (NCDF), scheme-on-read, metadados, FAIR/VAULTIS |
| 4 | Abdelazez et al. | 2023 | AI Approach to Wargaming | *Caso de uso central*: IA para COAs e Jogo da Guerra (Step 4 do PPM); contexto brasileiro |
| 5 | Turner et al. | 2024 | AI for Operational-level Planning (MSC2) | *Validação e método*: IA para Design Operacional; dados como barreira empírica; metodologia DSR |

Os cinco artigos cobrem o espectro necessário para fundamentar a dissertação do ponto de vista do domínio C2: do **o que fazer** (Artigo 1) ao **como integrar os dados** (Artigo 2), passando pelos **princípios arquiteturais** de dados militares (Artigo 3), pelo **caso de uso operacional** que justifica a proposta (Artigo 4) e pela **evidência de que a lacuna de dados é real e reconhecida** (Artigo 5). A estrutura arquitetural do Data Lake — camadas de ingestão, Delta Lake, catálogo, padrões de governança — é fundamentada pela literatura de engenharia de dados em seção distinta desta pesquisa, complementando o referencial de C2 aqui apresentado.
