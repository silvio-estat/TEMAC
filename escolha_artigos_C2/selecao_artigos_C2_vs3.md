...
### Versão 3 — Ordenamento Completo por Compatibilidade com o Projeto
...
Após leitura diagonal de todos os 29 documentos, estabeleceu-se um **ordenamento por compatibilidade** com o problema de pesquisa desta dissertação com base em **julgamento qualitativo informado** — não em pontuação automática. Conforme a metodologia PRISMA, a avaliação de elegibilidade aplica os critérios definidos na etapa anterior mediante leitura dos textos completos (ou em diagonal, quando suficiente para o julgamento). O resultado é, portanto, um posicionamento relativo fundamentado nas dimensões temáticas da Tabela 2, não um escore calculado.
